...
- **Rise launch splash + PWA launch assets (2026-07-24).** Ported `Design System/Splash -
  ...
  sliding loader bar, and "Keep climbing".
  **In-app splash** (`components/RiseSplash.tsx`, mounted in the root layout): server-rendered
  so it covers the app on the first paint (no data, no flash of the app underneath), then
  fades out. **Shown once per browser session** (sessionStorage guard) — a cold launch (new
  tab, or a PWA opened from the home screen) gets it; refreshes while working do not. SPA
  route changes never remount it (root layout persists). Responsive: vertical lockup below
  760px, the horizontal web lockup above. CSS is namespaced `rise-*` in globals.css; honours
  `prefers-reduced-motion`. Verified in-browser at 1280px (row, 64px) and 375px (column,
  42px), covers viewport at z-9999, dismiss + session-guard both confirmed, no hydration
  warnings.
  **PWA launch surfaces** so the native launch flows into the in-app splash instead of a
  cream/white flash: `manifest.ts` `background_color` cream → **orange `#f26a1b`** (the colour
  behind Android's native splash and the standalone window pre-paint); the four app icons
  regenerated to **chevron-on-orange**; and **7 iOS `apple-touch-startup-image`s** (one per
  current iPhone, via `appleWebApp.startupImage`). Assets are generated by
  `apps/web/scripts/gen-splash-assets.mjs` (sharp, at the repo root; pure vector, no fonts) —
  committed PNGs, re-run if the mark changes.
  **Deliberately not changed:** `appleWebApp.statusBarStyle` stays `default` — the shell only
  pads `safe-area-inset-bottom`, so `black-translucent` would slide the header under the notch.
  **Brand drift flagged, not resolved:** the splash says **Rise**, but the app is **Elearning**
  everywhere else (title, `manifest.name`/`short_name`, shell) and the design system is
  **Ascent**. The icon is now the Rise chevron while the installed-app name still reads
  Elearning; Android's native splash shows "Elearning" under the chevron. Picking one brand
  is a product decision left to the user. Only `apps/web` (student PWA) was touched; admin
  was not.
