...
export const authApi = {
  register: async (data: { fullName: string; email: string; password: string }, remember = true) => {
...
/** The rich per-question fields shared by Question and TestQuestion editors. */
...
/** Payload for the new question fields (tags are sent as names). */
...
export interface AdminReport {
   ...
   * Optional: the apps deploy independently, so a newer build can talk to an API that
   * predates this field. Callers must fall back.
   */
  trendLabel?: string;
