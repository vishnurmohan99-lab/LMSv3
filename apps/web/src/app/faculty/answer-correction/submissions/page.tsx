...
export default function FacultySubmissionsQueuePage() {
  ...
  return (
    <div className="fade-in-up" style={{ padding: "30px 36px 60px" }}>
      <div style={{ fontSize: 22, fontWeight: 800, letterSpacing: -0.4, marginBottom: 4 }}>Grading queue</div>
      <p style={{ color: "var(--ink2)", fontSize: 14, marginBottom: 24 }}>All student submissions for Answer Correction questions.</p>
      ...
      {loading ? (
        ...
        <div style={{ display: "grid", gap: 10 }}>
          {submissions.map((s) => (
            <Link
              key={s.id}
              href={`/faculty/answer-correction/submissions/${s.id}`}
              style={{
                display: "flex",
