  ...
  return (
    <main className="fade-in" style={{ padding: "30px 40px 60px" }}>
      ...
      {loading ? (
        ...
        <p style={{ color: "var(--ink2)" }}>No tests yet. Create your first one above.</p>
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 18 }}>
          {tests.map((test) => (
            <Link
              key={test.id}
              href={`/faculty/tests/${test.id}`}
              ...
              {test.bannerUrl ? (
                <div
                  onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    openImage(test.bannerUrl!, test.title);
                  }}
                  style={{ position: "relative", height: 110, background: `url(${test.bannerUrl}) center/cover`, cursor: "pointer" }}
                >
