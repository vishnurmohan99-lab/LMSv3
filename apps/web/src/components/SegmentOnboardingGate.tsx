...
/**
 * Forced first-run gate: a student with no segment must pick their class (+ track, if the
 * class has subsegments) before the app is usable — the backend scopes every course list to
 * the student's profile segment. Rendered by StudentShell when `profile.segmentId` is unset;
 * calls `onDone` (which reloads the profile) once saved.
 */
