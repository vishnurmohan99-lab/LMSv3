...
function LessonPreview({ lesson }: { lesson: Lesson }) {
  ...
  if (lesson.type === "PDF") {
    return lesson.contentUrl ? (
      <iframe src={`${lesson.contentUrl}#toolbar=0`} style={{ width: "100%", height: 400, border: "1px solid var(--line)", borderRadius: 8 }} />
    ) : (
...
export default function ChapterDetailPage() {
  ...
  async function onUpdateLesson(lessonId: string, data: { title?: string; contentUrl?: string; liveAt?: string; transcript?: string; captionsVtt?: string; videoChapters?: string }) {
